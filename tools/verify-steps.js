...
async function run(w, h, label) {
  ...
  try {
    ...
    chk('按钮上显示总步数', /12/.test(btn.text || ''), btn.text);
    ...
    chk('每一步都有一格缩略图', open1.cells === 12, open1.cells + ' 格');
    ...
    // 点第 9 格 -> 跳到第 9 步并关闭面板
    await c.ev(`document.querySelectorAll('.scell')[8].click()`);
    // 平滑滚动要时间，等它停下来再读（电脑宽屏上尤其慢）
    await waitFor(c, `document.getElementById('count').textContent.trim().indexOf('9 /') === 0`, 16);
    await sleep(200);
    ...
    chk('点缩略图跳到那一步', /^9 \//.test(after.count), after.count);
    chk('跳完面板自动关掉', after.closed === true);

    // 再打开：当前步要高亮
    await c.ev(`document.querySelector('.stepsbtn').click()`);
    await sleep(500);
    chk('再打开时当前步高亮', (await c.ev(`(() => {
      const n = document.querySelector('.scell.now');
      return !!n && +n.dataset.i === 8;
    })()`)) === true);
